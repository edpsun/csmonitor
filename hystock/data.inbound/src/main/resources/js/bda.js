...
function filterDiv(div, fdata) {
    ...
    show_by_id = true;
    if (fdata.stockid) {
        if (fdata.stockid.indexOf(stockid) > -1
                || stockid.indexOf(fdata.stockid) > -1) {
            show_by_id = true;
        } else {
            show_by_id = false;
        }
    ...
    // alert(show_by_tag +"::::"+ show_by_rate_min +"::::"+ show_by_rate_max
    // +"::::"+ show_by_qnum_min +"::::"+ show_by_qnum_max);
    if (show_by_id && show_by_tag && show_by_rate_min && show_by_rate_max
            && show_by_qnum_min && show_by_qnum_max) {
        elid = div.attr('elid');
        div.show();
        total = total + 1;
        ...
        tipList = tipList + "0" + elid + '=Tags:' + div.attr('tags')
                + "#$hCRate:" + rate + "#$hQnum:" + qnum + "<br>";
    } else {
        div.hide();
    }
}

function doAnyFilter() {
    $('#listdiv').hide();
    var tagstr = $(':input[name=tags]').val();
    if (jQuery.trim(tagstr) == "" || jQuery.trim(tagstr) == ",") {
        return;
    } else {
        tags = tagstr.split(",");
        $('.stdiv').hide();
...
$(function() {
    ...
    $("#anyFilterButton").bind("click", function(event) {
        doAnyFilter();
    });
